...
const INITIAL_JS = `window.showSuccessAlert = function() {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    html, body { height: 100%; width: 100%; background-color: #0f172a; font-family: 'Segoe UI', system-ui, sans-serif; overflow: hidden; }
    .fullscreen-wrapper { display: flex; flex-direction: column; height: 100vh; width: 100vw; }
    .preview-action-bar { display: flex; justify-content: space-between; align-items: center; background-color: #0b0f19; padding: 0 24px; height: 56px; border-bottom: 1px solid #1f2937; }
    .preview-title-badge { font-weight: 600; font-size: 0.95rem; color: #38bdf8; display: flex; align-items: center; gap: 10px; }
    .close-preview-btn { background-color: #1e293b; color: #cbd5e1; border: 1px solid #334155; padding: 8px 18px; font-size: 0.85rem; font-weight: 600; border-radius: 6px; cursor: pointer; transition: all 0.2s; }
    .close-preview-btn:hover { background-color: #334155; color: #fff; }
    .iframe-render-holder { flex: 1; width: 100%; height: calc(100vh - 56px); border: none; background-color: #0f172a; }
  </style>
</head>
<body>
  <div class="fullscreen-wrapper">
    <div class="preview-action-bar">
      <div class="preview-title-badge">🚀 CodeCraft Pro Sandbox Preview</div>
      <button class="close-preview-btn" onclick="window.close()">⬅ Close & Back to Editor</button>
    </div>
    <iframe class="iframe-render-holder" srcdoc="${userSrcDoc}" sandbox="allow-scripts allow-modals allow-same-origin"></iframe>
  </div>
</body>
</html>`;
      ...
      <main className="main-content">
        <aside className="file-explorer">
          ...
          <ul className="file-list">
            <li className={`file-item ${activeTab === 'html' ? 'selected' : ''}`} onClick={() => setActiveTab('html')}>📄 index.html</li>
            <li className={`file-item ${activeTab === 'css' ? 'selected' : ''}`} onClick={() => setActiveTab('css')}>📄 styles.css</li>
            <li className={`file-item ${activeTab === 'js' ? 'selected' : ''}`} onClick={() => setActiveTab('js')}>📄 script.js</li>
          </ul>
        ...
        <section className="workspace-section">
          <div className="tab-navigation">
            <button className={`tab-btn ${activeTab === 'html' ? 'active html-tab' : ''}`} onClick={() => setActiveTab('html')}>index.html</button>
            <button className={`tab-btn ${activeTab === 'css' ? 'active css-tab' : ''}`} onClick={() => setActiveTab('css')}>styles.css</button>
            <button className={`tab-btn ${activeTab === 'js' ? 'active js-tab' : ''}`} onClick={() => setActiveTab('js')}>script.js</button>
          </div>
